...
Finish11:													@ After completion of LastBitZero1 function(aka label), branching to here to maintatin the flow of the execution
cmp r0,#1
beq LastBitOne1										@ Branching to LastBitOne1 if "x" bit is "1"
Finish12:													@ After completion of LastBitOne1 function(aka label), branching to here to maintatin the flow of the execution
b Finish													@ FINALLY FINISHING THE COMPUATION OF ADDITON, AFTER THIS BRANCH, MULTIPLICATION WILL START


lpfpAdd:							
stmfd sp!,{r2-r9,lr}						  @ Addtion starts from here, setting the stack pointer
...
signbitgen:
add r1, r1, #8										@ Advancing the memory address stored in r1 by 8x8 = 64bits, this is where the RESULTANT will be stored, as per question
...
ADD r2,r2,#1073741824							@ Now the 2nd MSB will be changed to 1, by the additon of 01000000.....32 zeros, thus changing the "00.Mantissa" to "01.Mantissa"
LSL r3,#16
